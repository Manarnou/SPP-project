...
for i in range((len(d1))):
    x,y,theta_min_n1=function(wavelength_um, 1 , d1[i], cond)
    for j in range((len(n3_))):
...
################### choice of metal, MOVING ANGLE ###############################
plt.figure() 


d_in=np.linspace(10,100,20)
d=d_in*1e-9

n3_=np.linspace(1.33, 1.38, 20)

theta_min1=np.zeros((len(n3_),len(d)))
...
for c in range(len(cond)):
    ...
    for j in range(len(d)):
        for i in range(len(n3_)):
            x,y, theta_min1[i,j]=function(wavelength_um, n3_[i], d[j],cond[c])
    for i in range(len(d)):
        st[i] = np.mean(np.diff(theta_min1[:,i])/np.diff(n3_))
...
##################### Choice of metal FIXED  ANGLE ######################
plt.figure() 

wavelength_um = 0.6328
lambda_ = wavelength_um* 10**(-6)
n3_=np.linspace(1,1.05, 10)
d_nm=np.linspace(20, 100, 50)
d=d_nm*1e-9
